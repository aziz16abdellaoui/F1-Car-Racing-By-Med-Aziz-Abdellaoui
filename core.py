...
class Simulation:
    ...
    def generate_cars_from_nns(self, nns, parameters, images, batch, labels_batch=None):
        ...
        for i in range(num_teams):
            ...
            for driver_index in range(2):  # Create two cars per team, one for each driver
                sprite = pyglet.sprite.Sprite(image, batch=batch)
                
                # Create label with correct parameters
                label = CarLabel(order=len(self.cars) + 1, team=name, driver_index=driver_index, batch=labels_batch)
                
                pos = (*self.track.cps_arr[self.track.spawn_index], self.track.spawn_angle)
                
                # Get mutated version of one of the best NNs
                self.cars.append(Car(
                    nn=nns[index_loop(len(self.cars), len(nns))],  # Cycle through NNs
                    pos=pos,
    ...
    # checkpoints are sorted in a loop so it is only looking for the next cp
    def update_car_checkpoint(self, car):
        ...
        if dist < 120:
            ...
            if car.score % len(self.track.cps_arr) == 0:
                car.laps += 1
                print(f"Car completed lap {car.laps}")
        else:
            car.dist_to_cp = dist
